...
void Graph::Dijkstra(unsigned int sourceID)
{
    ...
    priority_queue<pair<unsigned int, unsigned int>, vector<pair<unsigned int, unsigned int>>, greater<pair<unsigned int, unsigned int>>> unvisited;
    ...
    while (!unvisited.empty()) 
    {
        ...
        unsigned int currDist = unvisited.top().first;
        unvisited.pop();

        // Searches through the distance vector for the smallest weight as the next current Node
        for (pair<unsigned int, float> n : nodes[currNode].getAdjacentNodes()) {
